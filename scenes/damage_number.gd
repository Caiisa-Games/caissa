...
func configure(amount: int, result: Result, index: int, is_critical := false) -> void:
	...
	label.add_theme_font_size_override("font_size", 11 if is_critical else 9)
...
func _play(horizontal_offset: float, is_critical: bool) -> void:
	...
	scale = Vector2.ONE * (0.82 if is_critical else 0.70)
	modulate.a = 1.0
	...
	pop_tween.tween_property(self, "scale", Vector2.ONE * (1.34 if is_critical else 1.18), 0.12).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
	pop_tween.tween_property(self, "scale", Vector2.ONE * (1.10 if is_critical else 1.00), 0.18).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
